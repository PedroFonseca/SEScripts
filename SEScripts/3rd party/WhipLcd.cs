            ...
            for (int i = 0; i < cmd.args.Count; i++)
            {
                ...
                for (int subi = 0; subi < arg.sub.Count; subi++)
                {
                    ...
                    for (int j = 0; j < blocks.Count(); j++)
                    {
                        ...
                        inv.GetItems(items);
                        string itemType = (items.Count > 0 ? items[0].Type.SubtypeId.ToString() : "");
                        for (int idx = 0; idx < items.Count; idx++)
                        {
                            MyInventoryItem item = items[idx];

                            if (item.Type.SubtypeId.ToString() != itemType)
...
{ "ShipController", _GridTerminalSystem.GetBlocksOfType<IMyShipController> }
    ...
    public static class MM
    {
        ...
        public static string GetItemFullType(MyInventoryItem item)
        {
            string typeid = item.Type.TypeId.ToString();
            typeid = typeid.Substring(typeid.LastIndexOf('_') + 1);
            return item.Type.SubtypeId + " " + typeid;
        }
